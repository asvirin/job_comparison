...
def get_avarage_salary(salary_from, salary_to):
...
def predict_rub_salary_for_headhunter(vacancy_salary):
    if vacancy_salary is None or vacancy_salary['currency'] != 'RUR':
        ...
        return get_avarage_salary(vacancy_salary['from'], vacancy_salary['to'])


def predict_rub_salary_for_superjob(salary_from, salary_to):
    if (salary_from == 0 and salary_to == 0) or (salary_from is None and salary_to is None):
        return None
    else:
        return get_avarage_salary(salary_from, salary_to)
...
def get_vacancies_statistics_Headhunter(source, languages_list, head_table):
    ...
    for language in languages_list:
        ...
        while vacancies['found'] > len(vacancies_all_pages) <= (int(max_result)-int(result_per_page)):
            payload = {
                'specialization': specialization,
                'area': location,
                'period': period_search,
        ...
        if len(vacancies_all_pages) == 0:
            vacancies_statistics_list.append([language, 0, 0, 0])
        else:
            vacancies_statistics_list.append(get_language_statistics(language, source, vacancies_all_pages))
            
    table_for_print = get_table_for_print(source, vacancies_statistics_list)

    return table_for_print


def get_vacancies_statistics_SuperJob(source, languages_list, head_table):
    ...
    for language in languages_list:
        ...
        vacancies_all_pages = vacancies['objects']
